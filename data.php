...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['delete'])) {
        ...
        if (move_uploaded_file($_FILES["photo"]["tmp_name"], $target_file)) {
            $photo = $target_file;
            $sql = "INSERT INTO reviews (destination_name, review, photo) VALUES ('$destination_name', '$review', '$photo')";
            $conn->query($sql);
        } else {
        ...
        $sql = "UPDATE reviews SET destination_name='$destination_name', review='$review', photo='$photo' WHERE id='$id'";
        $conn->query($sql);
    }
}
...
<head>
    ...
    <script>
        function editReview(id, destination_name, review, photo) {
            document.getElementById("editForm").style.display = "block";
            document.getElementById("edit_id").value = id;
            document.getElementById("edit_destination_name").value = destination_name;
...
<nav>
        ...
        <a href="index.php">TRAVEL REVIEW</a>
        </div>
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            echo "<div>";
            echo "<button onclick=\"editReview('" . $row['id'] . "','" . addslashes($row['destination_name']) . "','" . addslashes($row['review']) . "','" . $row['photo'] . "')\">Edit</button>";
            echo "<h2>" . $row['destination_name'] . "</h2>";
            echo "<img src='" . $row['photo'] . "' alt='" . $row['destination_name'] . "' width='600'>";
            echo "<p>" . $row['review'] . "</p>";
